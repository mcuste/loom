...
// fakeSpec is a hand-rolled RuntimeSpec used across runtime tests. Construct
// with newFake; whitelisted models/efforts and SystemPrompt support are
// captured by value. The zero value is also usable as an opaque placeholder
// for tests that never call Validate (see registry_test.go).
...
// TestSpecValidate exercises the RuntimeSpec.Validate contract directly on a
// fake spec, with no registry involvement. Dispatch-level errors (missing or
// unknown runtime) and runtime-name wrapping live in registry_test.go.
