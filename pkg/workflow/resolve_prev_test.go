...
func TestSubstitutePrevHit(t *testing.T) {
	...
	got := workflow.Substitute("refine {{prev.draft}}", nil, nil, nil, prev)
	if got != "refine v1" {
...
func TestSubstitutePrevMissCollapsesToEmpty(t *testing.T) {
	got := workflow.Substitute("before {{prev.draft}} after", nil, nil, nil, nil)
	if got != "before  after" {
...
func TestSubstitutePrevDoesNotCollideWithTaskNamespace(t *testing.T) {
	...
	got := workflow.Substitute("{{draft}}/{{prev.draft}}", outputs, nil, nil, prev)
...
func TestSubstitutePrevDoesNotCollideWithParamNamespace(t *testing.T) {
	...
	got := workflow.Substitute("{{params.draft}}/{{prev.draft}}", nil, params, nil, prev)
...
func TestSubstitutePrevNoDoubleExpansion(t *testing.T) {
	...
	got := workflow.Substitute("got {{prev.x}}", outputs, nil, nil, prev)
	if got != "got {{a}}" {
