...
// analysisScope bundles the immutable scope threaded through semantic analysis:
// the global task-id set used for dependency and placeholder
// validation, the param name set, and per-loop metadata derived from loop
// declarations.
...
func analyzeTaskDeclarations(st *analysisScope, tasks []taskDecl) (map[TaskID]taskAnalysis, error) {
	out := make(map[TaskID]taskAnalysis, len(tasks))
	for _, task := range tasks {
		analysis, err := analyzeTaskDeclaration(st, task)
		if err != nil {
			return nil, err
		}
		out[task.id] = analysis
	}
	return out, nil
}

// analyzeTaskDeclaration performs every per-task validation pass after the
// declaration model has a complete workflow scope: body shape, dependency graph
// edges, placeholder references, schema eligibility, when-expression, and
// runtime-routing placeholders. Lowering later only materializes the already
// analyzed task into a TaskNode.
func analyzeTaskDeclaration(st *analysisScope, rt taskDecl) (taskAnalysis, error) {
	tid := rt.id
	...
	case len(presentForms) > 1:
		return taskAnalysis{}, &TaskBodyConflictError{Task: tid, Fields: presentForms}
	case len(presentForms) == 0:
		return taskAnalysis{}, fmt.Errorf("task %q: %w", tid, ErrMissingPromptOrCommand)
	}
	// with: is only meaningful alongside workflow:.
	if rt.body.hasWith() && !rt.body.isSubWorkflow() {
		return taskAnalysis{}, fmt.Errorf("task %q: with: is only valid on a workflow task", tid)
	}
	// args: is only meaningful alongside script:.
	if rt.body.hasArgs() && !rt.body.isScript() {
		return taskAnalysis{}, fmt.Errorf("task %q: %w", tid, ErrArgsWithoutScript)
	}
	...
	return taskAnalysis{withArgs: withArgs, meta: meta}, nil
}
...
func buildTaskMeta(st *analysisScope, loop LoopID, tid TaskID, rt taskDecl, body string, withArgs []WithArg) (taskMeta, error) {
