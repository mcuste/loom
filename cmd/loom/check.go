...
func newCheckCmd(env *cliEnv) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return doCheck(cmd.OutOrStdout(), env.home, env.cwd, env.catalog, args[0], paramArgs)
		},
	}
...
func doCheck(w io.Writer, home, cwd string, catalog runtime.Catalog, path string, paramArgs []string) (err error) {
	...
	defer finish(&err)
	_, err = validateAndPlan(r, wf, paramInputs{cli: paramArgs}, catalog, true, nil)
	return err
}
